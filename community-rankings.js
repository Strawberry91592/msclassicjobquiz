(() => {
  ...
  const api = () => String(window.STATS_API_URL || '').replace(/\/$/, '');
  ...
  if (!wrap || !themeToggle) return;

  let toggle = document.getElementById('communityToggle');
  if (!toggle) {
  ...
  if (document.getElementById('communityRankingsModal')) return;
  ...
  const statsPanel = modal.querySelector('#communityRankingsStats');
  const updatedLabel = modal.querySelector('#communityUpdated');
  const closeBtn = modal.querySelector('.community-close-btn');
  let pollTimer = null;
  let refreshInFlight = false;

  function render(data) {
    const totals = Object.fromEntries(JOBS.map(([key]) => [key, Number(data?.totals?.[key] || 0)]));
    const total = Number(data?.total || 0);
    const sorted = JOBS.slice().sort((a,b) => totals[b[0]] - totals[a[0]] || a[1].localeCompare(b[1]));
    const max = Math.max(1, ...sorted.map(([key]) => totals[key]));
    statsPanel.innerHTML = `<div class="community-rank-summary">${total} completed quizzes counted</div>` + sorted.map(([key,name,family], index) => {
      const count = totals[key];
      const pct = total ? count / total * 100 : 0;
      const width = count / max * 100;
      return `<article class="community-rank-row ${index === 0 ? 'community-rank-top' : ''}">
        <div class="community-rank-number">#${index + 1}</div>
        <div class="community-rank-job"><strong>${escapeHtml(name)}</strong><span>${escapeHtml(family)}</span><div class="community-rank-meter"><i style="width:${width.toFixed(2)}%"></i></div></div>
        <div class="community-rank-total"><b>${count}</b><span>${pct.toFixed(1)}%</span></div>
      </article>`;
    }).join('');
  }
  ...
  async function refresh() {
    ...
    const url = api();
    if (!url) {
      statsPanel.innerHTML = '<div class="community-rank-state"><strong>Community results are not connected.</strong><span>The quiz still works normally.</span></div>';
      updatedLabel.textContent = 'Unavailable';
      return;
    }
    ...
    try {
      ...
      if (!statsPanel.children.length) statsPanel.innerHTML = '<div class="community-rank-state"><strong>Community results are unavailable right now.</strong><span>You can still take the quiz normally.</span></div>';
      updatedLabel.textContent = 'Unavailable';
    } finally {
  ...
  toggle.addEventListener('click', () => modal.hidden ? open() : close());
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    else if (!modal.hidden) startPolling();
  });
})();
